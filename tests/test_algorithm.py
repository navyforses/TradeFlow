...
import tradeflow.api
from tradeflow.api import FixedSlippage
from tradeflow.assets import Equity, Future, Asset
from tradeflow.assets.continuous_futures import ContinuousFuture
from tradeflow.assets.synthetic import (
...
from tradeflow.finance.commission import PerShare, PerTrade
from tradeflow.finance.execution import LimitOrder
from tradeflow.finance.order import ORDER_STATUS
from tradeflow.finance.trading import SimulationParameters
...
from tradeflow.finance.controls import AssetDateBounds
from tradeflow.testing import (
    FakeDataPortal,
...
from tradeflow.testing import RecordBatchBlotter
import tradeflow.testing.fixtures as zf
from tradeflow.test_algorithms import (
...
from tradeflow.testing.predicates import assert_equal
from tradeflow.utils.api_support import ZiplineAPI
from tradeflow.utils.context_tricks import CallbackManager, nop_context
...
import tradeflow.utils.factory as factory
...
class TestRecord(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_record_incr(self):
        ...
        def handle_data(self, data):
            ...
            tradeflow.api.record(name, self.incr, 'name2', 2, name3=self.incr)
...
class TestMiscellaneousAPI(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_cancel_policy_outside_init(self):
        code = """
from tradeflow.api import cancel_policy, set_cancel_policy
...
"""
    ...
    def test_cancel_policy_invalid_param(self):
        code = """
from tradeflow.api import set_cancel_policy
    ...
    def test_zipline_api_resolves_dynamically(self):
        ...
        for method in algo.all_api_methods():
            ...
            with ZiplineAPI(algo):
                self.assertIs(sentinel, getattr(tradeflow.api, name)())

    def test_sid_datetime(self):
        algo_text = """
from tradeflow.api import sid, get_datetime
...
"""
    ...
    def test_datetime_bad_params(self):
        algo_text = """
from tradeflow.api import get_datetime
...
def handle_data(context, data):
    ...
    def test_get_environment(self):
        ...
        def initialize(algo):
            self.assertEqual('tradeflow', algo.get_environment())
...
def log_nyse_close(context, data):
    context.nyse_closes.append(get_datetime())
        ...
        erroring_algotext = dedent(
            """
            from tradeflow.api import schedule_function
...
class TestSetSymbolLookupDate(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_set_symbol_lookup_date(self):
        ...
        """
        set_symbol_lookup_date = tradeflow.api.set_symbol_lookup_date

        def initialize(context):
            set_symbol_lookup_date(self.asset_ends[0])
            self.assertEqual(tradeflow.api.symbol('DUP').sid, self.sids[0])

            set_symbol_lookup_date(self.asset_ends[1])
            self.assertEqual(tradeflow.api.symbol('DUP').sid, self.sids[1])
...
class TestBeforeTradingStart(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_data_in_bts_minute(self):
        algo_code = dedent("""
        from tradeflow.api import record, sid
        def initialize(context):
    ...
    def test_portfolio_bts(self):
        algo_code = dedent("""
        from tradeflow.api import order, sid, record
    ...
    def test_account_bts(self):
        algo_code = dedent("""
        from tradeflow.api import order, sid, record, set_slippage, slippage
...
class TestAlgoScript(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_fixed_slippage(self):
        ...
        test_algo = self.make_algo(
            script="""
from tradeflow.api import (slippage,
                         commission,
                         set_slippage,
                         set_commission,
                         order,
...
def handle_data(context, data):
    ...
    def test_incorrectly_set_futures_slippage_model(self):
        code = dedent(
            """
            from tradeflow.api import set_slippage, slippage
    ...
    def test_batch_market_order_matches_multiple_manual_orders(self):
        ...
        multi_test_algo = self.make_algo(
            script=dedent("""\
                ...
                from tradeflow.api import sid, order
        ...
        batch_test_algo = self.make_algo(
            script=dedent("""\
                ...
                from tradeflow.api import sid, batch_market_order
    ...
    def test_order_dead_asset(self):
        ...
        self.run_algorithm(
            script="""
from tradeflow.api import order, sid
...
def handle_data(context, data):
    order(sid(0), 10)
        ...
        for order_str in ["order_value", "order_percent"]:
            test_algo = self.make_algo(
                script="""
from tradeflow.api import order_percent, order_value, sid
...
def handle_data(context, data):
    ...
    def test_schedule_function_time_rule_positionally_misplaced(self):
        ...
        algocode = dedent("""
        from tradeflow.api import time_rules, schedule_function
...
class TestCapitalChanges(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_capital_changes_daily_mode(self, change_type, value):
        ...
        algocode = """
from tradeflow.api import set_slippage, set_commission, slippage, commission, \
