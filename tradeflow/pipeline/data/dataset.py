...
from tradeflow.currency import Currency
from tradeflow.data.fx import DEFAULT_FX_RATE
from tradeflow.pipeline.classifiers import Classifier, Latest as LatestClassifier
from tradeflow.pipeline.domain import Domain, GENERIC
from tradeflow.pipeline.factors import Factor, Latest as LatestFactor
from tradeflow.pipeline.filters import Filter, Latest as LatestFilter
from tradeflow.pipeline.sentinels import NotSpecified, sentinel
from tradeflow.pipeline.term import (
    AssetExists,
    LoadableTerm,
    validate_dtype,
)
from tradeflow.utils.formatting import s, plural
from tradeflow.utils.input_validation import (
    coerce_types,
    ensure_dtype,
    expect_types,
)
from tradeflow.utils.numpy_utils import float64_dtype, NoDefaultMissingValue
from tradeflow.utils.preprocess import preprocess
from tradeflow.utils.string_formatting import bulleted_list
...
class BoundColumn(LoadableTerm):
    ...
    latest : tradeflow.pipeline.LoadableTerm
        A :class:`~tradeflow.pipeline.Filter`, :class:`~tradeflow.pipeline.Factor`,
        or :class:`~tradeflow.pipeline.Classifier` computing the most recently
        known value of this column on each date.
        See :class:`tradeflow.pipeline.mixins.LatestMixin` for more details.
    dataset : tradeflow.pipeline.data.DataSet
    ...
    of :class:`~tradeflow.pipeline.data.DataSet`. For example,
    :attr:`~tradeflow.pipeline.data.EquityPricing.close` is an instance of this
    ...
    def fx(self, currency):
        ...
        Parameters
        ----------
        currency : str or tradeflow.currency.Currency
...
class DataSetMeta(type):
    ...
    def specialize(self, domain):
        ...
        Parameters
        ----------
        domain : tradeflow.pipeline.domain.Domain
            Domain to which we should generate a specialization.

        Returns
        -------
        specialized : type
            A new :class:`~tradeflow.pipeline.data.DataSet` subclass with the
...
class DataSet(with_metaclass(DataSetMeta, object)):
    ...
    1. A collection of :class:`~tradeflow.pipeline.data.Column` objects that
       describe the queryable attributes of the dataset.

    2. A :class:`~tradeflow.pipeline.domain.Domain` describing the assets and
    ...
    :data:`~tradeflow.pipeline.domain.GENERIC`, which means that they can be used
    ...
    EquityPricing. It is defined as::
        ...
        from tradeflow.pipeline.domain import US_EQUITIES
    ...
    def get_column(cls, name):
        ...
        Returns
        -------
        column : tradeflow.pipeline.data.BoundColumn
...
class DataSetFamilySlice(DataSet):
    ...
    :class:`tradeflow.pipeline.data.dataset.DataSetFamily` objects
    """
...
class DataSetFamily(with_metaclass(DataSetFamilyMeta)):
    ...
    :class:`~tradeflow.pipeline.data.DataSet` objects, each of which has the same
    columns, domain, and ndim.

    :class:`DataSetFamily` objects are defined with one or more
    :class:`~tradeflow.pipeline.data.Column` objects, plus one additional field:
    ``extra_dims``.
    ...
    :meth:`~tradeflow.pipeline.data.DataSetFamily.slice` method.
