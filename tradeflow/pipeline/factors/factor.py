...
from tradeflow.utils.compat import wraps
from tradeflow.errors import (
    BadPercentileBounds,
    UnknownRankMethod,
    UnsupportedDataType,
)
from tradeflow.lib.normalize import naive_grouped_rowwise_apply
from tradeflow.lib.rank import masked_rankdata_2d, rankdata_1d_descending
from tradeflow.pipeline.api_utils import restrict_to_dtype
from tradeflow.pipeline.classifiers import Classifier, Everything, Quantiles
from tradeflow.pipeline.dtypes import (
    CLASSIFIER_DTYPES,
    FACTOR_DTYPES,
    FILTER_DTYPES,
)
from tradeflow.pipeline.expression import (
    BadBinaryOperator,
    COMPARISONS,
    is_comparison,
    MATH_BINOPS,
...
from tradeflow.pipeline.filters import (
    Filter,
    NumExprFilter,
    PercentileFilter,
    MaximumFilter,
)
from tradeflow.pipeline.mixins import (
    CustomTermMixin,
    LatestMixin,
...
from tradeflow.pipeline.sentinels import NotSpecified, NotSpecifiedType
from tradeflow.pipeline.term import AssetExists, ComputableTerm, Term
from tradeflow.utils.functional import with_doc, with_name
from tradeflow.utils.input_validation import expect_types
...
from tradeflow.utils.numpy_utils import (
    as_column,
    bool_dtype,
    coerce_to_dtype,
    float64_dtype,
...
from tradeflow.utils.sharedoc import templated_docstring
...
Construct a :class:`~tradeflow.pipeline.{rtype}` computing ``self {op} other``.

Parameters
----------
other : tradeflow.pipeline.Factor, float
...
BINOP_RETURN_FILTER = """\
filter : tradeflow.pipeline.Filter
    Filter computing ``self {op} other`` with the outputs of ``self`` and
    ``other``.
"""

BINOP_RETURN_FACTOR = """\
factor : tradeflow.pipeline.Factor
...
CORRELATION_METHOD_NOTE = dedent(
    ...
    as inputs to windowed :class:`~tradeflow.pipeline.Factor` objects. Examples
    of such expressions include This includes
    :class:`~tradeflow.pipeline.data.BoundColumn`
    :class:`~tradeflow.pipeline.factors.Returns` and any factors created from
    :meth:`~tradeflow.pipeline.Factor.rank` or
    :meth:`~tradeflow.pipeline.Factor.zscore`.
    """
)
...
def summary_method(name):
    ...
    def f(self, mask=NotSpecified):
        ...
        Parameters
        ----------
        mask : tradeflow.pipeline.Filter, optional
        ...
        Returns
        -------
        result : tradeflow.pipeline.Factor
        """
        return DailySummary(
            func,
            self,
...
class Factor(RestrictedDTypeMixin, ComputableTerm):
    ...
    Factors can also be converted into :class:`tradeflow.pipeline.Filter` objects
    ...
    def demean(self, mask=NotSpecified, groupby=NotSpecified):
        ...
        groupby : tradeflow.pipeline.Classifier, optional
    ...
    def zscore(self, mask=NotSpecified, groupby=NotSpecified):
        ...
        Returns
        -------
        zscored : tradeflow.pipeline.Factor
        ...
        Examples
        --------
        See :meth:`~tradeflow.pipeline.Factor.demean` for an in-depth
    ...
    def rank(self,
        ...
        Returns
        -------
        ranks : tradeflow.pipeline.Factor
    ...
    def pearsonr(self, target, correlation_length, mask=NotSpecified):
        ...
        Parameters
        ----------
        target : tradeflow.pipeline.Term
        ...
        Returns
        -------
        correlations : tradeflow.pipeline.Factor
        ...
        :class:`tradeflow.pipeline.factors.RollingPearsonOfReturns`
    ...
    def linear_regression(self, target, regression_length, mask=NotSpecified):
        ...
        Returns
        -------
        regressions : tradeflow.pipeline.Factor
    ...
    def winsorize(self,
        ...
        Returns
        -------
        winsorized : tradeflow.pipeline.Factor
    ...
    def quantiles(self, bins, mask=NotSpecified):
        ...
        Returns
        -------
        quantiles : tradeflow.pipeline.Classifier
    ...
    def quartiles(self, mask=NotSpecified):
        ...
        Returns
        -------
        quartiles : tradeflow.pipeline.Classifier
    ...
    def quintiles(self, mask=NotSpecified):
        ...
        Returns
        -------
        quintiles : tradeflow.pipeline.Classifier
    ...
    def deciles(self, mask=NotSpecified):
        ...
        Returns
        -------
        deciles : tradeflow.pipeline.Classifier
    ...
    def isnan(self):
        ...
        Returns
        -------
        nanfilter : tradeflow.pipeline.Filter
        """
        return self != self
...
class GroupedRowTransform(Factor):
    ...
    mask : tradeflow.pipeline.Filter
        Mask of entries to ignore when calculating transforms.
    groupby : tradeflow.pipeline.Classifier
    ...
    See Also
    --------
    tradeflow.pipeline.Factor.zscore
    tradeflow.pipeline.Factor.demean
    tradeflow.pipeline.Factor.rank
    """
    window_length = 0
