...
from tradeflow.errors import IncompatibleTerms
from tradeflow.pipeline.factors import CustomFactor
from tradeflow.pipeline.filters import SingleAsset
from tradeflow.pipeline.mixins import StandardOutputs
from tradeflow.pipeline.sentinels import NotSpecified
from tradeflow.pipeline.term import AssetExists
from tradeflow.utils.input_validation import (
    expect_bounded,
    expect_dtypes,
    expect_types,
)
from tradeflow.utils.math_utils import nanmean
...
class RollingPearson(_RollingCorrelation):
    ...
    Parameters
    ----------
    base_factor : tradeflow.pipeline.Factor
        The factor for which to compute correlations of each of its columns
        with `target`.
    target : tradeflow.pipeline.Term with a numeric dtype
...
class RollingSpearman(_RollingCorrelation):
    ...
    :class:`tradeflow.pipeline.factors.RollingSpearmanOfReturns`
...
class RollingLinearRegression(CustomFactor):
    ...
    Parameters
    ----------
    dependent : tradeflow.pipeline.Factor
        The factor whose columns are the predicted/dependent variable of each
        regression with `independent`.
    independent : tradeflow.pipeline.slice.Slice or tradeflow.pipeline.Factor
    ...
    :class:`tradeflow.pipeline.factors.RollingLinearRegressionOfReturns`
...
class RollingLinearRegressionOfReturns(RollingLinearRegression):
    ...
    :class:`tradeflow.pipeline.CustomFactor`.
...
def vectorized_pearson_r(dependents, independents, allowed_missing, out=None):
    ...
    See Also
    --------
    :class:`tradeflow.pipeline.factors.RollingPearson`
    :class:`tradeflow.pipeline.factors.RollingPearsonOfReturns`
    """
    nan = np.nan
    isnan = np.isnan
