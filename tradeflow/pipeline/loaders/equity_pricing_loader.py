...
from tradeflow.data.fx import ExplodingFXRateReader
from tradeflow.lib.adjusted_array import AdjustedArray
from tradeflow.utils.numpy_utils import repeat_first_axis
...
class EquityPricingLoader(implements(PipelineLoader)):
    ...
    Parameters
    ----------
    raw_price_reader : tradeflow.data.session_bars.SessionBarReader
        Reader providing raw prices.
    adjustments_reader : tradeflow.data.adjustments.SQLiteAdjustmentReader
        Reader providing price/volume adjustments.
    fx_reader : tradeflow.data.fx.FXRateReader
    ...
    def _inplace_currency_convert(self, columns, arrays, dates, sids):
        ...
        Parameters
        ----------
        columns : list[tradeflow.pipeline.data.BoundColumn]
    ...
    def _split_column_types(self, columns):
        ...
        Returns
        -------
        ohlcv_columns : list[tradeflow.pipeline.data.BoundColumn]
            Price and volume columns from ``columns``.
        currency_columns : list[tradeflow.pipeline.data.BoundColumn]
