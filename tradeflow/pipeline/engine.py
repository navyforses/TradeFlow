...
1. Determine the domain of the pipeline. The domain determines the
   ...
   tradeflow.pipeline.domain.infer_domain.
...
2. Build a dependency graph of all terms in `pipeline`, with
   ...
   tradeflow.pipeline.graph.TermGraph and
   tradeflow.pipeline.graph.ExecutionPlan.
...
from tradeflow.lib.adjusted_array import ensure_adjusted_array, ensure_ndarray
...
from tradeflow.utils.pandas_utils import explode
...
from tradeflow.utils.date_utils import compute_date_range_chunks
from tradeflow.utils.pandas_utils import categorical_df_concat
...
class PipelineEngine(with_metaclass(ABCMeta)):
    ...
    def run_pipeline(self, pipeline, start_date, end_date, hooks=None):
        ...
        Parameters
        ----------
        pipeline : tradeflow.pipeline.Pipeline
            The pipeline to run.
        ...
        result : pd.DataFrame
            ...
            instances of :class:`tradeflow.pipeline.Term`.
...
class SimplePipelineEngine(PipelineEngine):
    ...
    populate_initial_workspace : callable, optional
        ...
        :func:`tradeflow.pipeline.engine.default_populate_initial_workspace`
        for more info.
    ...
    def compute_chunk(self,
        ...
        Parameters
        ----------
        graph : tradeflow.pipeline.graph.ExecutionPlan
