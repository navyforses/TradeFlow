...
from tradeflow.errors import UnsupportedDataType
from tradeflow.lib.labelarray import LabelArray
from tradeflow.lib.quantiles import quantiles
...
from tradeflow.pipeline.term import ComputableTerm
from tradeflow.utils.compat import unicode
from tradeflow.utils.input_validation import expect_types, expect_dtypes
...
class Classifier(RestrictedDTypeMixin, ComputableTerm):
    ...
    def to_workspace_value(self, result, assets):
        ...
        This is the inverse of :func:`~tradeflow.pipeline.term.Term.postprocess`.
        """
        if self.dtype == int64_dtype:
...
class Relabel(SingleInputMixin, Classifier):
    ...
    Parameters
    ----------
    arg : tradeflow.pipeline.Classifier
...
class CustomClassifier(PositiveWindowLengthMixin,
    ...
    tradeflow.pipeline.CustomFilter
    """
    def _validate(self):
        try:
...
class Latest(LatestMixin, CustomClassifier):
    ...
    See Also
    --------
    tradeflow.pipeline.data.dataset.BoundColumn.latest
    """
    pass
