...
from tradeflow.lib.labelarray import LabelArray, labelarray_where
from tradeflow.utils.context_tricks import nop_context
from tradeflow.utils.input_validation import expect_dtypes, expect_types
from tradeflow.utils.numpy_utils import bool_dtype
from tradeflow.utils.pandas_utils import nearest_unequal_elements
...
class LatestMixin(SingleInputMixin):
    """
    Common behavior for :attr:`tradeflow.pipeline.data.BoundColumn.latest`.

    Given a :class:`~tradeflow.pipeline.data.DataSet` named ``MyData`` with a
    ...
    resulting expression will be a :class:`~tradeflow.pipeline.CustomFilter` for
    boolean columns, and the resulting object will be a
    :class:`~tradeflow.pipeline.CustomClassifier` for string or integer columns.
    """
    window_length = 1
