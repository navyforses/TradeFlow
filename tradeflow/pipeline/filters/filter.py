...
from tradeflow.lib.rank import is_missing, grouped_masked_is_maximal
...
from tradeflow.pipeline.term import ComputableTerm, Term
...
class Filter(RestrictedDTypeMixin, ComputableTerm):
    ...
    requested computation. For example, :meth:`tradeflow.pipeline.Factor.top`
    ...
    operators (``<``, ``<=``, ``!=``, ``eq``, ``>``, ``>=``) of
    :class:`~tradeflow.pipeline.Factor`. For example, a natural way to construct
    ...
    20.0::
        ...
        >>> from tradeflow.pipeline.factors import VWAP
    ...
    **either** of its inputs produced True.
    ...
    def if_else(self, if_true, if_false):
        ...
        Parameters
        ----------
        if_true : tradeflow.pipeline.term.ComputableTerm
            Expression whose values should be used at locations where this
            filter outputs True.
        if_false : tradeflow.pipeline.term.ComputableTerm
            Expression whose values should be used at locations where this
            filter outputs False.

        Returns
        -------
        merged : tradeflow.pipeline.term.ComputableTerm
...
class NullFilter(SingleInputMixin, Filter):
    ...
    Parameters
    ----------
    factor : tradeflow.pipeline.Term
...
class PercentileFilter(SingleInputMixin, Filter):
    ...
    Parameters
    ----------
    factor : tradeflow.pipeline.factor.Factor
...
class CustomFilter(PositiveWindowLengthMixin, CustomTermMixin, Filter):
    ...
    :class:`~tradeflow.pipeline.CustomFactor` for more details on
    implementing a custom ``compute`` method.

    See Also
    --------
    tradeflow.pipeline.CustomFactor
    """
    def _validate(self):
        try:
...
class MaximumFilter(Filter, StandardOutputs):
    ...
    def __new__(cls, factor, groupby, mask):
        if groupby is NotSpecified:
            from tradeflow.pipeline.classifiers import Everything
            groupby = Everything()
