...
import tradeflow.utils.paths as pth
from tradeflow.extensions import load
from tradeflow.errors import SymbolNotFound
from tradeflow.algorithm import TradingAlgorithm, NoBenchmark
from tradeflow.finance.blotter import Blotter
...
def _run(handle_data,
    ...
    This is shared between the cli and :func:`tradeflow.run_algo`.
    """
    ...
    try:
        ...
        raise _RunAlgoError(
            (
                ...
                ' ``tradeflow.api.set_benchmark`` was not called in'
                ' ``initialize``.'
            ),
            (
                "Neither '--benchmark-symbol' nor '--benchmark-sid' was"
                " provided, and ``tradeflow.api.set_benchmark`` was not called"
...
def run_algorithm(start,
    ...
    metrics_set : iterable[Metric] or str, optional
        ...
        resolve the set with :func:`tradeflow.finance.metrics.load`.
    ...
    blotter : str or tradeflow.finance.blotter.Blotter, optional
        Blotter to use with this algorithm. If passed as a string, we look for
        a blotter construction function registered with
        ``tradeflow.extensions.register`` and call it with no parameters.
        Default is a :class:`tradeflow.finance.blotter.SimulationBlotter` that
    ...
    See Also
    --------
    tradeflow.data.bundles.bundles : The available data bundles.
    """
...
class BenchmarkSpec(object):
    ...
    tradeflow.utils.run_algo.run_algorithm.
