...
from tradeflow._protocol import handle_non_market_minutes
from tradeflow.errors import (
...
from tradeflow.finance.blotter import SimulationBlotter
from tradeflow.finance.controls import (
    LongOnly,
    MaxOrderCount,
    MaxOrderSize,
    MaxPositionSize,
...
from tradeflow.finance.execution import (
    LimitOrder,
    MarketOrder,
    StopLimitOrder,
    StopOrder,
...
from tradeflow.finance.cancel_policy import NeverCancel, CancelPolicy
from tradeflow.finance.asset_restrictions import (
    NoRestrictions,
    StaticRestrictions,
    SecurityListRestrictions,
)
from tradeflow.assets import Asset, Equity, Future
from tradeflow.gens.tradesimulation import AlgorithmSimulator
from tradeflow.finance.metrics import MetricsTracker, load as load_metrics_set
from tradeflow.pipeline import Pipeline
import tradeflow.pipeline.domain as domain
from tradeflow.pipeline.engine import (
    ExplodingPipelineEngine,
    SimplePipelineEngine,
)
from tradeflow.utils.api_support import (
    api_method,
    require_initialized,
    require_not_initialized,
    ZiplineAPI,
    disallowed_in_before_trading_start)
from tradeflow.utils.compat import ExitStack
from tradeflow.utils.input_validation import (
    coerce_string,
    ensure_upper_case,
    error_keywords,
    expect_dtypes,
...
from tradeflow.utils.numpy_utils import int64_dtype
from tradeflow.utils.pandas_utils import normalize_date
from tradeflow.utils.cache import ExpiringCache
from tradeflow.utils.pandas_utils import clear_dataframe_indexer_caches

import tradeflow.utils.events
from tradeflow.utils.events import (
    EventManager,
    make_eventrule,
    date_rules,
    time_rules,
...
from tradeflow.utils.math_utils import (
    tolerant_equals,
    round_if_near_integer,
)
from tradeflow.utils.preprocess import preprocess
from tradeflow.utils.security_list import SecurityList

import tradeflow.protocol
from tradeflow.sources.requests_csv import PandasRequestsCSV

from tradeflow.gens.sim_engine import MinuteSimulationClock
from tradeflow.sources.benchmark_source import BenchmarkSource
from tradeflow.zipline_warnings import ZiplineDeprecationWarning
    ...
    *args, **kwargs
    ...
    def __init__(self,
        ...
        self.event_manager.add_event(
            tradeflow.utils.events.Event(
                tradeflow.utils.events.Always(),
...
""".strip().format(class_name=self.__class__.__name__,
        ...
        field : {'platform', 'arena', 'data_frequency',
            ...
            The field to query. The options have the following meanings:
              ...
              platform : str
                  ...
                  will be the string 'tradeflow'. This can allow algorithms to
    ...
    def fetch_csv(self,
        ...
        **kwargs
        ...
        Returns
        -------
        csv_data_source : tradeflow.sources.requests_csv.PandasRequestsCSV
    ...
    def add_event(self, rule, callback):
        ...
        """
        self.event_manager.add_event(
            tradeflow.utils.events.Event(rule, callback),
        )
    ...
    def schedule_function(self,
        ...
        date_rule : tradeflow.utils.events.EventRule, optional
            Rule for the dates on which to execute ``func``. If not
            passed, the function will run every trading day.
        time_rule : tradeflow.utils.events.EventRule, optional
        ...
        See Also
        --------
        :class:`tradeflow.api.date_rules`
        :class:`tradeflow.api.time_rules`
        """
    ...
    def set_benchmark(self, benchmark):
        ...
        Parameters
        ----------
        benchmark : tradeflow.assets.Asset
    ...
    def continuous_future(self,
        ...
        Returns
        -------
        continuous_future : tradeflow.assets.ContinuousFuture
    ...
    def symbol(self, symbol_str, country_code=None):
        ...
        Returns
        -------
        equity : tradeflow.assets.Equity
        ...
        See Also
        --------
        :func:`tradeflow.api.set_symbol_lookup_date`
        """
    ...
    def symbols(self, *args, **kwargs):
        ...
        *args : iterable[str]
        ...
        Returns
        -------
        equities : list[tradeflow.assets.Equity]
    ...
    def future_symbol(self, symbol):
        ...
        Returns
        -------
        future : tradeflow.assets.Future
    ...
    def order(self,
        ...
        See Also
        --------
        :class:`tradeflow.finance.execution.ExecutionStyle`
        :func:`tradeflow.api.order_value`
        :func:`tradeflow.api.order_percent`
        """
    ...
    def order_value(self,
        ...
        Notes
        -----
        See :func:`tradeflow.api.order` for more information about
        ``limit_price``, ``stop_price``, and ``style``

        See Also
        --------
        :class:`tradeflow.finance.execution.ExecutionStyle`
        :func:`tradeflow.api.order`
        :func:`tradeflow.api.order_percent`
        """
        if not self._can_order_asset(asset):
            return None
    ...
    def set_slippage(self, us_equities=None, us_futures=None):
        ...
        :func:`~tradeflow.api.initialize`.

        See Also
        --------
        :class:`tradeflow.finance.slippage.SlippageModel`
        """
        if self.initialized:
    ...
    def set_commission(self, us_equities=None, us_futures=None):
        ...
        See Also
        --------
        :class:`tradeflow.finance.commission.PerShare`
        :class:`tradeflow.finance.commission.PerTrade`
        :class:`tradeflow.finance.commission.PerDollar`
        """
        if self.initialized:
    ...
    def set_cancel_policy(self, cancel_policy):
        ...
        See Also
        --------
        :class:`tradeflow.api.EODCancel`
        :class:`tradeflow.api.NeverCancel`
        """
    ...
    def order_target(self,
        ...
        :class:`tradeflow.finance.execution.ExecutionStyle`
        :func:`tradeflow.api.order`
        :func:`tradeflow.api.order_target_percent`
        :func:`tradeflow.api.order_target_value`
        """
        if not self._can_order_asset(asset):
            return None
    ...
    def order_target_value(self,
        ...
        :class:`tradeflow.finance.execution.ExecutionStyle`
        :func:`tradeflow.api.order`
        :func:`tradeflow.api.order_target`
        :func:`tradeflow.api.order_target_percent`
        """
        if not self._can_order_asset(asset):
            return None
    ...
    def cancel_order(self, order_param):
        ...
        if isinstance(order_param, tradeflow.protocol.Order):
    ...
    def set_do_not_order_list(self, restricted_list, on_error='fail'):
        ...
        if isinstance(restricted_list, SecurityList):
            ...
            warnings.warn(
                ...
                "Create a tradeflow.finance.asset_restrictions."
    ...
    def set_asset_restrictions(self, restrictions, on_error='fail'):
        ...
        See Also
        --------
        tradeflow.finance.asset_restrictions.Restrictions
        """
    ...
    def attach_pipeline(self, pipeline, name, chunks=None, eager=True):
        ...
        See Also
        --------
        :func:`tradeflow.api.pipeline_output`
        """
        if chunks is None:
    ...
    def pipeline_output(self, name):
        ...
        See Also
        --------
        :func:`tradeflow.api.attach_pipeline`
        :meth:`tradeflow.pipeline.engine.PipelineEngine.run_pipeline`
        """
        try:
