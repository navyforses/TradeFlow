...
__ https://github.com/quantopian/tradeflow/issues
__ https://github.com/
__ https://groups.google.com/forum/#!forum/tradeflow
...
.. code-block:: bash
   ...
   $ git clone git@github.com:your-github-username/tradeflow.git
...
.. code-block:: bash
   ...
   $ mkvirtualenv tradeflow
   $ etc/dev-install

After installation, you should be able to use the ``tradeflow`` command line interface from your virtualenv:
...
.. code-block:: bash
   ...
   # where tradeflow is the name of your virtualenv
   $ deactivate tradeflow
   $ workon tradeflow
...
If you want to work with tradeflow using a `Docker`__ container, you'll need to build the ``Dockerfile`` in the Zipline root directory, and then build ``Dockerfile-dev``. Instructions for building both containers can be found in ``Dockerfile`` and ``Dockerfile-dev``, respectively.
...
__ https://travis-ci.org/quantopian/tradeflow
__ https://ci.appveyor.com/project/quantopian/tradeflow
...
If you update the tradeflow codebase so that it now depends on a new version of a library,
...
otherwise the lockfile will not meet the constraints specified to pip by tradeflow
at install time (via ``etc/requirements.in`` via ``setup.py``).

__ https://github.com/quantopian/tradeflow/tree/master/etc/requirements_locked.txt

If the tradeflow codebase can still support an old version of a dependency, but you want
...
We have a set of `whatsnew <https://github.com/quantopian/tradeflow/tree/master/docs/source/whatsnew>`__ files that are used for documenting changes that have occurred between different versions of Zipline.
