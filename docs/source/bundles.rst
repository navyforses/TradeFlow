...
- ``quandl`` (provided by tradeflow, though deprecated)
- ``quantopian-quandl`` (provided by tradeflow, the default bundle)
...
The ingestion process will invoke some custom bundle command and then write the data to a standard location that tradeflow can find.
By default the location where ingested data will be written is ``$ZIPLINE_ROOT/data/<bundle>`` where by default ``ZIPLINE_ROOT=~/.tradeflow``.
The ingestion step may take some time as it could involve downloading and processing a lot of data.
To ingest a bundle, run:

.. code-block:: bash

   $ tradeflow ingest [-b <bundle>]
...
.. code-block:: bash
   ...
   $ tradeflow clean [-b <bundle>] --before <date>

   # clean everything newer than <date>
   $ tradeflow clean [-b <bundle>] --after <date>

   # keep everything in the range of [before, after] and delete the rest
   $ tradeflow clean [-b <bundle>] --before <date> --after <after>

   # clean all but the last <int> runs
   $ tradeflow clean [-b <bundle>] --keep-last <int>
...
.. code-block:: bash
   ...
   $ tradeflow run --bundle <bundle> --algofile algo.py ...
...
By default tradeflow comes with the ``quantopian-quandl`` data bundle which uses quandl's `WIKI dataset <https://www.quandl.com/data/WIKI>`_.
...
.. code-block:: bash
   ...
   $ tradeflow ingest -b quantopian-quandl
   $ tradeflow ingest
...
tradeflow. To add a new bundle, one must implement an ``ingest`` function.

The ``ingest`` function is responsible for loading the data into memory and
passing it to a set of writer objects provided by tradeflow to convert the data to
tradeflow's internal format. The ingest function may work by downloading data from
...
``asset_db_writer`` is an instance of :class:`~tradeflow.assets.AssetDBWriter`.
...
:meth:`~tradeflow.assets.AssetDBWriter.write` with dataframes for the various
...
:class:`~tradeflow.data.minute_bars.BcolzMinuteBarWriter`. This writer is used to
convert data to tradeflow's internal bcolz format to later be read by a
:class:`~tradeflow.data.minute_bars.BcolzMinuteBarReader`. If minute data is
provided, users should call
:meth:`~tradeflow.data.minute_bars.BcolzMinuteBarWriter.write` with an iterable of
...
empty iterator to :meth:`~tradeflow.data.minute_bars.BcolzMinuteBarWriter.write`
to signal that there is no minutely data.

.. note::

   The data passed to
   :meth:`~tradeflow.data.minute_bars.BcolzMinuteBarWriter.write` may be a lazy
...
:class:`~tradeflow.data.bcolz_daily_bars.BcolzDailyBarWriter`. This writer is
used to convert data into tradeflow's internal bcolz format to later be read by a
:class:`~tradeflow.data.bcolz_daily_bars.BcolzDailyBarReader`. If daily data is
provided, users should call
:meth:`~tradeflow.data.minute_bars.BcolzDailyBarWriter.write` with an iterable of
(sid dataframe) tuples. The ``show_progress`` argument should also be forwarded
...
iterable to :meth:`~tradeflow.data.minute_bars.BcolzMinuteBarWriter.write` to
...
:class:`~tradeflow.data.adjustments.SQLiteAdjustmentWriter`. This writer is
used to store splits, mergers, dividends, and stock dividends. The data should
be provided as dataframes and passed to
:meth:`~tradeflow.data.adjustments.SQLiteAdjustmentWriter.write`. Each of
...
:class:`tradeflow.utils.calendars.TradingCalendar`. The calendar is provided to
...
``cache`` is an instance of :class:`~tradeflow.utils.cache.dataframe_cache`. This
...
:class:`~tradeflow.utils.cli.maybe_show_progress`. This argument should always be
...
purposes in ``tradeflow/tests/resources/csvdir_samples``.
...
``~/.tradeflow/extension.py`` and import the csvdir bundle, along with ``pandas``.

.. code-block:: python

	 import pandas as pd

	 from tradeflow.data.bundles import register
	 from tradeflow.data.bundles.csvdir import csvdir_equities
...
.. code-block:: bash
	 ...
	 $ tradeflow ingest -b custom-csvdir-bundle
	 ...
	 $ CSVDIR=/path/to/your/csvs tradeflow ingest -b custom-csvdir-bundle


If you would like to use equities that are not in the NYSE calendar, or the existing tradeflow calendars,
