...
The source can be found at: https://github.com/quantopian/tradeflow
...
This tutorial assumes that you have tradeflow correctly installed, see the
`installation
instructions <https://github.com/quantopian/tradeflow#installation>`__ if
you haven't set up tradeflow yet.

Every ``tradeflow`` algorithm consists of two functions you have to
define:

* ``initialize(context)``
* ``handle_data(context, data)``

Before the start of the algorithm, ``tradeflow`` calls the
...
After the algorithm has been initialized, ``tradeflow`` calls the
...
.. code-block:: python
   ...
   from tradeflow.examples import buyapple
   buyapple??


.. code-block:: python

   from tradeflow.api import order, record, symbol
...
``tradeflow.api``. Here we are using :func:`~tradeflow.api.order()` which takes two
arguments: a security object, and a number specifying how many stocks you would
like to order (if negative, :func:`~tradeflow.api.order()` will sell/short
...
Finally, the :func:`~tradeflow.api.record` function allows you to save the value
...
with :func:`~tradeflow.api.record` under the name you provided (we will see this
...
To now test this algorithm on financial data, ``tradeflow`` provides three
...
After you installed tradeflow you should be able to execute the following
...
``buyapple_out.pickle``, we call ``tradeflow run`` as follows:

.. code-block:: bash

    tradeflow run -f ../tradeflow/examples/buyapple.py --start 2016-1-1 --end 2018-1-1 -o buyapple_out.pickle --no-benchmark
...
After each call to ``handle_data()`` we instruct ``tradeflow`` to order 10
stocks of AAPL. After the call of the ``order()`` function, ``tradeflow``
enters the ordered stock and amount in the order book. After the
``handle_data()`` function has finished, ``tradeflow`` looks for any open
...
slippage model that ``tradeflow`` uses, see the `Quantopian
...
rows. Note that ``tradeflow`` makes heavy usage of ``pandas``, especially
...
quantitative researchers ``tradeflow`` provides an easy way to run your
algorithm inside the Notebook without requiring you to use the CLI.

To use it you have to write your algorithm in a cell and let ``tradeflow``
know that it is supposed to run this algorithm. This is done via the
``%%tradeflow`` IPython magic command that is available after you
``import tradeflow`` from within the IPython Notebook. This magic takes
...
execute the following cell after importing ``tradeflow`` to register the
magic.
...
.. code-block:: python
   ...
   %%tradeflow --start 2016-1-1 --end 2018-1-1
   from tradeflow.api import symbol, order, record
...
.. code-block:: python
   ...
   %%tradeflow --start 2014-1-1 --end 2018-1-1 -o dma.pickle
...
architecture, API, and features of ``tradeflow``. For next steps, check
out some of the
`examples <https://github.com/quantopian/tradeflow/tree/master/tradeflow/examples>`__.

Feel free to ask questions on `our mailing
list <https://groups.google.com/forum/#!forum/tradeflow>`__, report
problems on our `GitHub issue
tracker <https://github.com/quantopian/tradeflow/issues?state=open>`__,
`get
involved <https://github.com/quantopian/tradeflow/wiki/Contribution-Requests>`__,
