...
When we are ready to ship a new release of tradeflow, edit the :doc:`releases`
...
example, we generate stub files for the :mod:`~tradeflow.api` namespace, since that
...
.. note::
   ...
   ``tradeflow.api``, this is done in a header string in the
   ``gen_type_stubs.py`` script mentioned above.  If new classes are added as
   parameters or return types of ``tradeflow.api`` functions, then new imports
...
Next, click the "Draft a new release" button on the `tradeflow releases page
<https://github.com/quantopian/tradeflow/releases>`__.  For the new release,
...
from the tradeflow root. This will create a gzipped tarball that includes all the
python, cython, and miscellaneous files needed to install tradeflow. To test that
...
.. code-block:: bash
   ...
   $ pip install <tradeflow-root>/dist/tradeflow-<major>.<minor>.<micro>.tar.gz
   $ python -c 'import tradeflow;print(tradeflow.__version__)'
...
.. code-block:: bash
   ...
   $ pip install twine
   $ twine upload --repository-url https://test.pypi.org/legacy/ dist/tradeflow-<version-number>.tar.gz
...
This will upload tradeflow to the pypi test server. To test installing from pypi,
create a new virtualenv, ``cd`` into a clean directory and then run:

.. code-block:: bash

   $ pip install --extra-index-url https://test.pypi.org/simple tradeflow
   $ python -c 'import tradeflow;print(tradeflow.__version__)'
...
.. code-block:: bash
   ...
   $ twine upload dist/tradeflow-<version-number>.tar.gz

``bdist``
^^^^^^^^^

Because tradeflow now supports multiple versions of numpy, we're not building
...
To update `tradeflow.io <https://www.tradeflow.io>`__, checkout the
latest master and run:

.. code-block:: python

    python <tradeflow_root>/docs/deploy.py

This will build the documentation, checkout a fresh copy of the ``gh-pages``
git branch, and copy the built docs into the tradeflow root.
...
.. code-block:: bash
   ...
   $ git add .
   $ git commit -m "DOC: update tradeflow.io"
   $ git push origin gh-pages

`tradeflow.io <https://www.tradeflow.io>`__ will update in a few moments.

Uploading conda packages
~~~~~~~~~~~~~~~~~~~~~~~~

`Travis <https://travis-ci.org/quantopian/tradeflow>`__ and
`AppVeyor <https://ci.appveyor.com/project/quantopian/tradeflow/branch/master>`__
build tradeflow conda packages for us (for Linux/OSX and Windows respectively).
Once they have built and uploaded to anaconda.org the tradeflow packages for the
...
tradeflow's dependencies. You can do this from the
...
To build the conda packages for tradeflow locally, run:
