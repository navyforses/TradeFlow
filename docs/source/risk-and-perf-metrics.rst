...
.. code-block:: bash
   ...
   $ tradeflow run algorithm.py -s 2014-01-01 -e 2014-02-01 --metrics-set my-metrics-set

``run_algorithm``
`````````````````

When running through the :func:`~tradeflow.run_algorithm` interface, the metrics
...
.. code-block:: bash
   ...
   $ tradeflow run algorithm.py -s 2014-01-01 -e 2014-02-01 --metrics-set none
...
``ledger`` is an instance of :class:`~tradeflow.finance.ledger.Ledger` which is
...
:class:`~tradeflow.utils.calendars.TradingCalendar` which is the trading calendar
...
:class:`~tradeflow.sources.benchmark_source.BenchmarkSource` which is the
interface to the returns of the benchmark specified by
:func:`~tradeflow.api.set_benchmark`.
...
``data_portal`` is an instance of :class:`~tradeflow.data.data_portal.DataPortal`
...
Users may use :func:`tradeflow.finance.metrics.register` to register a new metrics
