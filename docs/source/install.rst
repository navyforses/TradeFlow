...
For windows, the easiest and best supported way to install tradeflow is to use
:ref:`Conda <conda>`.
...
.. code-block:: bash
    ...
    conda install -c conda-forge tradeflow
...
.. code-block:: bash
    ...
    $ conda create -n env_tradeflow python=3.6


Now you have set up an isolated environment called ``env_tradeflow``, a sandbox-like
structure to install Zipline. Then you should activate the conda environment
by using the command

.. code-block:: bash

    $ conda activate env_tradeflow

You can install Zipline by running

.. code-block:: bash

    (env_tradeflow) $ conda install -c conda-forge tradeflow

.. note::

    The ``conda-forge`` channel so far only has tradeflow 1.4.0+ packages for python 3.6.
    Conda packages for previous versions of tradeflow for pythons 2.7/3.5/3.6 are
    still available on Quantopian's anaconda channel, but are not being updated.
    They can be installed with:

    .. code-block:: bash

        (env_tradeflow35) $ conda install -c Quantopian tradeflow

To deactivate the ``conda`` environment:

.. code-block:: bash

    (env_tradeflow) $ conda deactivate
